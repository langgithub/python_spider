...
![在线爬虫](在线爬虫设计.png)
    ...
    * 进入初始化爬虫（分发器），主线程启动单线程轮询以上阶段完成爬虫任务
        > 读取redis中存放爬虫阶段
        > 根据阶段进入相应爬虫，返回相依response）
        > 调用getSeleniumServerStateFromJson，根据response设置爬虫阶段状态
    * 主线程启动单线程后，轮询等待结果
        > 读取redis中存放爬虫阶段
        > 判断阶段 if "FAIL" return seleniumCrawlResponse.errorCode
        > 判断阶段 if "WAIT_CODE" return seleniumCrawlResponse.errorCode(这个值在爬虫后调用getSeleniumServerStateFromJson中都会更新成NONE，或者程序异常)
        > 判断阶段 if "SUCCESS" return ErrorCode.SUCCESS
        > result 有状态返回，结束
